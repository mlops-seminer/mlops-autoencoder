# オートエンコーダによる異常検知モデル
本実習はオートエンコーダーで画像の異常検知を行うタスクです。プログラムは完成しています。ハイパーパラメータチューニングによるPoCと実験・モデル管理、本番環境における推論精度の確認、データの再収集とモデルの再学習までが体験できます。ローカルでMLflowサーバーが起動していることを前提としています。

最終的に本番環境でリアルタイム推論のテストをするのがゴールです。
